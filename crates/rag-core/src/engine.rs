...
mod tests {
    ...
    #[test]
    #[cfg(feature = "persistence")]
    fn test_load_corrupted_index_triggers_reindex() {
        use tempfile::TempDir;
        let temp_dir = TempDir::new().unwrap();
        let mut engine = RagEngine::new(MockBackend);

        // Write a corrupted file
        let path = crate::persistence::index_path(temp_dir.path(), "mock-embed");
        std::fs::write(&path, r#"{"version": 2, "chunks": {"corrupt": "#).unwrap();

        engine.load_from_dir(temp_dir.path()).unwrap();

        assert!(
            engine.needs_reindex,
            "Should mark for reindex on corruption"
        );
